@echo off
echo ==========================================================================
echo FIXING AND PUSHING BLOODLINK PROJECT TO GITHUB
...
echo Step 1: Removing any existing git repositories...
echo - Removing main .git directory if it exists
if exist .git (
    rmdir /s /q .git
    echo   - Main .git directory removed.
) else (
    echo   - No main .git directory found.
)

echo - Checking for nested .git directories
if exist frontend\.git (
    echo   - Removing frontend\.git directory
    rmdir /s /q frontend\.git
)

if exist backend\.git (
    echo   - Removing backend\.git directory
    rmdir /s /q backend\.git
)

:: Additional check for any other nested .git directories
